...
async def upsert_user(
    pool: asyncpg.Pool, telegram_id: int, username: str | None, full_name: str
) -> None:
...
async def set_delivery_type(
    pool: asyncpg.Pool, telegram_id: int, delivery_type: str
) -> None:
    await pool.execute(
        "UPDATE users SET delivery_type = $1 WHERE id = $2", delivery_type, telegram_id
    )


async def set_room_number(pool: asyncpg.Pool, telegram_id: int, room: str) -> None:
    await pool.execute(
        "UPDATE users SET room_number = $1 WHERE id = $2", room, telegram_id
    )
...
async def get_product(pool: asyncpg.Pool, product_id: int) -> asyncpg.Record | None:
    return await pool.fetchrow(
        "SELECT * FROM products WHERE id = $1 AND in_stock = TRUE", product_id
    )
...
async def get_user_by_ref_code(
    pool: asyncpg.Pool, ref_code: str
) -> asyncpg.Record | None:
...
async def set_ref_code(pool: asyncpg.Pool, user_id: int, ref_code: str) -> None:
    await pool.execute(
        "UPDATE users SET ref_code = $1 WHERE id = $2", ref_code, user_id
    )
...
async def cart_change_qty(
    pool: asyncpg.Pool, user_id: int, product_id: int, delta: int
) -> int:
...
async def admin_edit_product_name(
    pool: asyncpg.Pool, product_id: int, name: str
) -> None:
    await pool.execute("UPDATE products SET name = $1 WHERE id = $2", name, product_id)


async def admin_edit_product_price(
    pool: asyncpg.Pool, product_id: int, price: int
) -> None:
    await pool.execute(
        "UPDATE products SET price = $1 WHERE id = $2", price, product_id
    )
...
async def get_all_messages(
    pool: asyncpg.Pool, offset: int = 0, limit: int = 10
) -> list[asyncpg.Record]:
    return await pool.fetch(
        """
...
async def clear_all_messages(pool: asyncpg.Pool) -> int:
    """Удаляет все сообщения. Возвращает количество удалённых."""
    result = await pool.execute("DELETE FROM user_messages")
    try:
